...
def Get_Data(path,tag=""):
    ...
    for folder1 in os.listdir(path):
        ...
        for item in os.listdir(path + '/'+ folder1):
            #将tuber 和 normal part放在一起，然后对于每个item先检测名字中是否包含tuber
            if item.split("_")[1].split(".")=="tuber":
                path_img = path + '/' + folder1
                #tuber img
                tuber_img = load_img(path_img + '/' + item)
                input_tensor = tf_img(tuber_img)  # 3x400x225 -> 3x299x299 size may differ
                input_tensor = input_tensor.unsqueeze(0)  # 3x299x299 -> 1x3x299x299
                input = torch.autograd.Variable(input_tensor, requires_grad=False)
                output_features = model.features(input)
                tuber_x=output_features.view(1,-1)[0]
                print("size of tuber x",tuber_x,len(tuber_x))

                #normal img
                normal_img=load_img(path_img+'/'+item.split("_")[0]+"_normal.jpg")
                ...
                input2 = torch.autograd.Variable(input_tensor2, requires_grad=False)
                output_features2 = model.features(input2)
                normal_x = output_features2.view(1, -1)[0]
                print("size of normal x",normal_x,len(normal_x))
                #x=output_logits[0]

                x_numpy=np.array(tuber_x)-np.array(normal_x)
                y = cls_num
                data.append(x_numpy)
                target.append(y)
                print(item)
...
def train(epoch, net, trainLoader, optimizer):
    ...
    #save the model
    torch.save(model,"echo_model.pkl")
